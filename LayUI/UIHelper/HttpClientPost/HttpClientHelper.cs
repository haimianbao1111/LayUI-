...
namespace UIHelper
{
    public class HttpClientHelper
    {
        ...
        /// <returns></returns>
        public static string postMessage(string strUrl, List<PostDateClass> postParaList, bool isFile = false)
        {
            if (isFile == true)
            {
        ...
        public static string postFileMessage(string strUrl, List<PostDateClass> postParaList)
        {
            try
            {
                ...
                //如果上传失败，同时抓包发现http头部多了个Expect: 100-continue 请用以下方式关掉100-continue
